/*
   S Box - Forward - Matrix
*/
static FORWARD_M: [[u8; 16]; 16] = [
    [
...
/*
   S Box - Forward
*/
pub fn forward(byte: u8) -> u8 {
    let byte_b: [u8; 8] = crate::modules::aes::utils::binary::integer_to_binary(byte);

    let mut first_n: [u8; 4] = [0; 4];
    let mut last_n: [u8; 4] = [0; 4];
    ...
    let row_i: usize = crate::modules::aes::utils::binary::binary_to_integer(first_n).into();
    let column_i: usize = crate::modules::aes::utils::binary::binary_to_integer(last_n).into();

    return FORWARD_M[row_i][column_i];
}
